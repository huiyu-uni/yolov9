...
from utils.metrics import ConfusionMatrix, ap_per_class_voc, box_iou
...
def run(
    ...
    seen = 0
    # confs = np.arange(0.1, 1, 0.05)
    # confusion_matrices = [ConfusionMatrix(nc, _conf, 0.6) for _conf in confs]
    ...
    if len(stats) and stats[0].any():
        tp, fp, p, r, f1, ap, ap_class = ap_per_class_voc(*stats, plot=plots, save_dir=save_dir, names=names)
    ...
    if save_json and len(jdict):
        ...
        try:  # https://github.com/cocodataset/cocoapi/blob/master/PythonAPI/pycocoEvalDemo.ipynb
            ...
            eval.accumulate()
            eval.summarize()
            map, map50, map75 = eval.stats[:3]  # update results (mAP@0.5:0.95, mAP@0.5, mAP@0.75)
    ...
    # No need to return mAP75 as it will not be used for updating best model 
